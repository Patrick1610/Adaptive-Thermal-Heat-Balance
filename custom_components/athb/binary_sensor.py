...
class _SurfaceRiskBinarySensor(AthbEntity, BinarySensorEntity, RestoreEntity):
    """One restorable surface-risk signal."""
    ...
    @property
    def is_on(self) -> bool | None:
        value = self.runtime.values.get(self._key)
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if not isinstance(self.runtime.values.get(self._key), bool):
            restored = await self.async_get_last_state()
            if restored is not None and restored.state in {"on", "off"}:
                self._restored_is_on = restored.state == "on"


class SurfaceHighHumidityBinarySensor(_SurfaceRiskBinarySensor):
    _attr_translation_key = "surface_high_humidity"

    def __init__(self, runtime: ZoneRuntime) -> None:
        super().__init__(runtime, "surface_high_humidity")


class SurfaceSaturationBinarySensor(_SurfaceRiskBinarySensor):
...
async def async_setup_entry(
    ...
    if surface_enabled:
        entities.extend(
            (
                SurfaceHighHumidityBinarySensor(entry.runtime_data),
        ...
        obsolete_unique_ids = {
            f"{entry.runtime_data.zone_uuid}_surface_high_humidity",
            f"{entry.runtime_data.zone_uuid}_surface_saturation",
        }
